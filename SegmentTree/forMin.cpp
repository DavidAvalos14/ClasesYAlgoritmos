...
void build(vector<int>& nums, int idx, int l, int r) {
    ...
    seg[idx] = min(seg[idx*2], seg[idx*2+1]);   // Minimo de ese rango
...
void update(int idx, int pos, int val, int l, int r) {
    ...
    seg[idx] = min(seg[idx*2], seg[idx*2+1]);       // Actualizamos el minimo
}

long long query(int idx, int l, int r, int ql, int qr) {
    if(ql > r || qr < l) {                      // El rango buscado esta fuera del rango actual (LONG_MAX nunca será el minimo y no se actualiza)
        return LONG_MAX;
    }
    ...
    long long mid = (l + r) / 2;
    long long left = query(idx*2, l, mid, ql, qr);        // Guardamos el minimo del rango izquierdo
    long long right = query(idx*2+1, mid+1, r, ql, qr);   // Guardamos el minimo del rango derecho

    return (long long)min(left, right);                            // Retornamos el minimo de ambos rangos
}
